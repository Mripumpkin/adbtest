...
class ADBConnection(threading.Thread):
    ...
    def dump_file(self, f):
        ...
        obj = {
            #"event": "adbhoney.session.file_upload",
            #"src_ip": self.addr[0],
            "username": "",
            "protocol": "",
            "session": self.session,
            "time_stamp": int(time.time()),
            # change "input" to "message"
            "messge": cmd,
            "password": "",
            "event": "adbhoney.session.file_upload",
            ...
            # change
            "command": "",
            "dst_port": int(CONFIG.get('honeypot', 'port')),
            "dst_ip": localip,
            "shasum": sha256sum,
            "outfile": fp,
            "filename": f['name']
    ...
    def process_connection(self):
        ...
        obj = {
            ...
            "message":"",
            "password": "",
            "event": "adbhoney.session.connect",
            "src_port": self.addr[1],
            "service":"adb",
            "system":"",
            "src_ip": self.addr[0],
